...
Another interesting question is, whether more experienced developers are more likely or less
likely to introduce buggy commits. 

In this paper, we study the social characteristics of modern software to understand 
the correlation between these social characteristics with the quality of the software.  
...
that it is hard to know the ``bugginess" of a commit as bugs in the commit are often not 
...
start from bug fixes, and  extract the bug introduction time for these bug fixes 
by using the version control system. Then we can know collectively when these fixes 
are introduced. %, hence we can learn 
%at a certain time of the day (e.g., each hour), how many of the bugs are introduced.
In particular,
we leverage the ``git blame'' functionality to identify the last modification time of each 
buggy line. 
