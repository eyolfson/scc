...
for Linux), so that all of the depicted bug introduction rates are
meaningful.
...
\end{center}
\caption{Linux percentage of buggy commits (bars) and total number of commits (circles) versus time of day}
...
or single.  This information is only interesting for Linux, as the
...
the number of authors in each class; Linux has 49 day job authors,
and 238 weekly and 288 monthly authors.

Our results show that Linux developers who commit changes daily, but
...
Linux, 43 of the 50 bugs that we automatically identified as
...
non-bug-introducing commits from both Linux and
