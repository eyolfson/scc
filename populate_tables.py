...
parser.add_argument("--verify-removals", action='store_true', help="enable verification of fixing commit removals")
...
VERIFY_REMOVALS = args.verify_removals
LOG = args.log
    ...
    def analyze(self, git_commit):
        ...
        if len(git_commit.parents) == 0:
            ...
            if db_repository.first_commit:
                if db_current_commit.utc_time < db_repository.first_commit.utc_time:
                    ...
                    db_repository.save()
                    log_thread(thread, 'Changed "first_commit" to %s.' % sha1)
                else:
                    db_repository.first_commit = db_commit
                    db_repository.save()
                    log_thread(thread, 'Set "first_commit" to %s.' % sha1)
        ...
        def fini(filename, context):
            ...
            for line, add_removal in blame_lines:
                ...
                git_commit_introducing = git.Repo(directory, odbt=git.GitCmdObjectDB).commit(introducing_sha1)
...
def find_bugs(thread, commit):
    ...
    if len(commit.parents) == 0:
        lock_repository.acquire()
        if db_repository.first_rawcommit:
            if db_current_rawcommit.utc_time < db_repository.first_rawcommit.utc_time:
                db_repository.first_rawcommit = db_current_rawcommit
                db_repository.save()
    ...
    # Ignore any merges or commits which are not fixes
    if merge or not re_fixing_commit_message.search(commit.message):
        return

    # We do not need to use a lock here since this will only be called once
    # per fix commit
    db_bug = models.Bug.objects.get_or_create(fixing_rawcommit=db_current_rawcommit)[0]
    bug_introduction = {}

    diff_index = get_diffs(commit)
    for diff in diff_index:
        ...
        if not re_c_file.search(filename):
            continue
        ...
        # to the blame (which is the line before the add block in the previous
        # file) and ignore the rest of the block.
        for line in get_unified_diff(previous_lines, current_lines):
            if line.startswith("@@"):
                match = re_unified_diff_line_numbers.match(line)
                # Subtract one so our increment on the next line is correct
        ...
        try:
            ...
            thread.switch_to_revision(previous_rev)
            blame = thread.git.blame(["-l", "-s", "-w", "--root", previous_rev,
                                      filename]).replace('\r','').splitlines()
...
def merge_rawauthors():
    ...
    while len(email_dict) > 0:
        ...
        if len(rawauthors) > 0:
            ...
            if not author:
                ...
                author = models.Author.objects.create(repository=db_repositorysitory,
                                                      name=name,
                                                      email=email)
    ...
    def manual_merge_email_to_name(email, name):
        ...
        if merged_rawauthor_qs.count() > 0:
            ...
            author = models.Author.objects.create(repository=db_repositorysitory, name=name,
                                                  email=email)
        rawauthor.author = author
        rawauthor.save()
...
#     for c in models.RawCommit.objects.filter(rawauthor__repository=db_repositorysitory):
